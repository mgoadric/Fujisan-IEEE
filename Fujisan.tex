...
\title{Improving Solvability for Procedurally\\ Generated Challenges in Physical Solitare\\Games Through Embedded Mechanics}
%
%
...
\begin{abstract}
    Challenges for physical solitaire puzzle games are typically designed in advance by humans and limited in number. Alternately, some games incorporate stochastic setup rules, where the human solver randomly sets up the game board before solving the challenge. \hl{These setup rules} greatly increase the number of possible challenges, but can often generate unsolvable or uninteresting challenges. To better understand \hl{the compromises involved in minimizing undesirable challenges, we examine three games where component design choices can influence the stochastic nature of the resulting challenge generation algorithms.} We evaluate the effect of these components and algorithms on ease of physical setup, challenge solvability, and challenge difficulty. \hl{We find that algorithms which constrain randomness through using embedding sub-elements of the puzzle mechanics into the components can be simple for the solver to construct yet generate solvable and difficult challenges.}
\end{abstract}
...
\IEEEPARstart{P}{rocedural} content generation (PCG), where algorithms are used to create game content,\cite{PCGSURVEY} is increasingly being used to derive new challenges for a wide variety of games. Using the PCG taxonomy described by Togelius et al. \cite{SBPCG}, PCG can occur {\it offline} (beforehand) or {\it online} (dynamically during the game), the content can be {\it constructed}
...
Following the puzzle terminology of Browne \cite{PUZZLENATURE}, such that there is a setter who creates challenges and a solver who solves them, \hl{we can apply the above PCG terminology to puzzle design.} Setters typically {\it construct} their challenges {\it offline} in advance, using creative yet {\it deterministic} means. 
...
When designing such a random setup process to assemble a challenge, three questions naturally arise: {\it How easy is the setup process for the solver to execute? Can all such challenges be solved? How difficult are the created challenges? } \hl{Since these puzzle challenges are created by the solver, they are restricted to using only the components included in the puzzle. However, as much of PCG research involves digital games and puzzles, where algorithms are only limited by computational time and space constraints, there is little research into the relationship between the choice and design of physical puzzle components and PCG}. 

\hl{To understand this relationship, we examine three different styles of physical solitaire games that incorporate} {\it online, stochastic, generate-and-test} \hl{PCG algorithms for challenge setup: Fujisan\footnote{http://www.ludism.org/ppwiki/Fuji-san}, a transportation puzzle, BoxOff} \cite{BoxOffGAMES} \hl{, a token removal puzzle, and Pretzel} \cite{de1981pretzel} \hl{, a card arrangement puzzle.} We provide detailed descriptions of different \hl{game components and} challenge setup algorithms \hl{for each game}, then compare and contrast these multiple setup algorithms and constraints using computational simulations. Finally, we evaluate these algorithms with respect to ease of player setup, solvability, and difficulty. We find that \hl{those component choices
and algorithms which intentionally incorporate desirable constraints based on each puzzle's mechanics are more successful across all three metrics}.
...
\subsubsection{Shuffled}

Fujisan was created specifically for the piecepack game system\footnote{http://www.ludism.org/ppwiki/SolitaryConfinement}. The piecepack is a set of board game parts that can be used to design and play a wide variety of games. \hl{The mountain in Fujisan was constructed with game tiles representing a 2$\times$2 grid. The values were added using 24 round coins, which represented the cross product of two sets: suits (sun, moon, crown, arms) and values (0, 1, 2, 3, 4, 5). Values are indicated on the front of the coin, while suits are found on the back.}
...
\subsubsection{piecepack}

The original published Fujisan ruleset was devised to address the issue of double 0 steps, adding the constraint that each step must have two different values. This was achieved by using the suit information embedded on the backs of the coins.
...
Another way to generate Fujisan challenges is to combine the values with the 2$\times$2 tiles, engraving
...
can be used to construct Fujisan challenges. If we eliminate those dominoes that include a 6, along with all doubles, we are left with 15 dominoes. 
...
\textbf{Shuffled}                 & 3                                            & 4                                                  & 1*                                                 \\ \hline
\textbf{piecepack}                & 4                                            & 3                                                  & 1*                                                 \\ \hline
\textbf{Engraved}                 & 2                                            & 1*                                                  & 4                                                 \\ \hline
...
\begin{figure}[t]
\centering
\includegraphics[width=8.8cm]{boxoff6x82.png}
\caption{6$\times$8 BoxOff challenge with three colors.}
\label{fig:boxoffbig}
\end{figure}

BoxOff is a 2D token removal puzzle designed by Stephen Myers.  \cite{BoxOffGAMES} Challenges for this puzzle consist of a 6$\times$8 grid of squares filled with exactly 16 each of three colors, as shown in 
Figure \ref{fig:boxoffbig}. The solver can eliminate two squares of the same color at a time if either of the following two rules apply.
\begin{enumerate}
...
\end{enumerate}
The challenge is solved when every square in the grid has been eliminated.

SHOW A DIAGRAM OF RULE 2!

Browne and Maire previously investigated altering the game design parameters for BoxOff using Monte Carlo simulation \cite{MCPUZZLE}. They explored multiple grid sizes and numbers of colors, ultimately finding that challenges generated for the original 6$\times$8 puzzle configuration described by Myers were highly solvable and robust against random solvers. Other configurations, either of smaller board sizes, or more colors, exhibited poor solvability. In particular, we will examine the 4$\times$6 board with four colors, where challenge solvability was less than 25\%, and the 6$\times$6 board with six colors, where challenge solvability was less than 3\%.
...
The components of the original 6$\times$8 physical BoxOff puzzle consist of 48 tokens evenly distributed among three colors. 
\subsubsection{Shuffled}
With these components, the solver can employ a simple shuffling algorithm to create a challenge.
\begin{quote}
    Shuffle the color tokens randomly, and arrange into a grid.
\end{quote}

HOW MANY CHALLENGES???

This shuffling works well on moderate sized boards with few colors, but as demonstrated by Browne and Maire, it suffers when applied to more colors, as shown in Figure \ref{fig:boxoff666}. The uniform nature of the shuffling appears to prevent enough critical mass of adjacent tokens of the same color that can fulfill Rule 1 above and be eliminated.

\begin{figure}[t]
\centering
\includegraphics[width=8.8cm]{boxoff66basic.png}
\caption{6$\times$6 BoxOff challenge with four colors, generated with a simple shuffle of the 36 color tokens. }
\label{fig:boxoff666}
\end{figure}
...
\subsubsection{Tiles}
To address this issue, we introduce a different level of abstraction for creating the challenge using L-shaped tiles, as shown in Figure \ref{fig:ltiles}.
Our goal in creating these tiles is encourage more adjacent tokens of the same color. However, we also must avoid creating situations where all tiles are adjacent to tokens of the same color, else the puzzle becomes to easy to solve. 

\begin{figure}[t]
\centering
\includegraphics[width=6.8cm]{boxoffltiles.png}
\caption{Sample L-tiles for BoxOff challenge creation. Tile can be rotated 180 degrees, and the dashed line shows how they can be flipped. }
\label{fig:ltiles}
\end{figure}


For both the 4$\times$6 and 6$\times$6 grids, each tile will have exactly two squares of the same color and one square of a different color. We then distribute the third square colors as even as possible using a cyclic arrangement of the colors, as shown in Figures \ref{fig:boxoff4x6cycle} and \ref{fig:boxoff6x6cycle}. To increase the number of possible challenges, these tiles can be rotated 180 degrees, as well as flipped to a mirror image along the center square of the L.
...
\begin{quote}
    Shuffle and flip the tiles randomly, then arrange into a grid of connected 3$\times$2 subgrids. 
\end{quote}

Figure \ref{fig:boxofftiles} shows a 4$\times$6 grid created with L-tiles. To physically play BoxOff with this tile setup, the solver will need to place neutral tokens on each square as they are eliminated, continuing until the board is full of tokens. In essence, this transforms the game from BoxOff to BoxOn, yet it retains the same ruleset.

\begin{figure}[t]
\centering
\includegraphics[width=4cm]{boxoff.png}
\caption{Cycle of color connections used to construct L-shaped tiles for 4$\times$6 BoxOff challenges with four colors.}
\label{fig:boxoff4x6cycle}
\end{figure}

\begin{figure}[t]
\centering
\includegraphics[width=5cm]{boxoff666.png}
\caption{Cycle of color connections used to construct L-shaped tiles for 6$\times$6 BoxOff challenges with six colors.}
\label{fig:boxoff6x6cycle}
\end{figure}


\begin{figure}[b]
\centering
\includegraphics[width=8.8cm]{boxoff46tiles.png}
\caption{4$\times$6 BoxOff challenge with four colors, generated using L-shaped tiles. }
...
We see increased in solvability!

\begin{figure}[t]
\includegraphics[width=8.8cm]{boxonsolve.png}
\caption{Effect of challenge generation algorithms on solvability for BoxOff 4$\times$6 with four colors.}
\label{fig:boxoffsolvediff}
\end{figure}

\begin{figure}[t]
\includegraphics[width=8.8cm]{6x6solvediff.png}
\caption{Effect of challenge generation algorithms on solvability for BoxOff 6$\times$6 with six colors.}
\label{fig:boxoff666solvediff}
\end{figure}

This comes at a cost of difficulty.
...
\begin{figure}[t]
\centering
\includegraphics[width=8.8cm]{pretzelbasic.png}
\caption{Pretzel solitaire challenge with four suits and four values, generated with a random shuffle.}
...
\subsubsection{Random Suits}

\subsubsection{Sequential Suits}

\subsection{Evaluation}

\begin{figure}[t]
\includegraphics[width=8.8cm]{pretzelsolve4x8.png}
\caption{Effect of challenge generation algorithms on solvability for BoxOff 4$\times$6 with four colors.}
\label{fig:boxoffsolvediff}
\end{figure}
...
Each game has a basic Shuffled strategy, simple components placed on a board.
