...
\hl{While some claim that solitaire games without the potential for a winning outcome are ``a rather sad form of amusing oneself,"} \cite{de1981pretzel} \hl{others find a ``catharsis [in] patience" even without perfect solvability} \cite{morehead2014complete}. Thus, when designing such a random setup process to assemble a challenge, \hl{two questions naturally arise for the designer:}{\it What percentage of such challenges can be solved? Are the created challenges interesting?} \hl{Also, since these puzzle challenges are instantiated by the solver, they are restricted to using only the components included in the puzzle. However, as much of PCG research involves digital games and puzzles, where algorithms are only limited by computational time and space constraints, there is little research into the relationship between the structural design of physical puzzle components and PCG}. 
...
THIS NEEDS WORK FOR BOTH (4, 6, 4) and (6, 6, 6)
...
\caption{Cycle of color connections used to construct L-shaped tiles for BoxOff challenges, (4, 6, 4) on the left, and (6, 6, 6) challenges on the right.}
...
\caption{A sample (4, 6, 4) BoxOff challenge, generated using L-shaped tiles. }
...
Similarly, Figure \ref{fig:boxoffsolvediff} also shows the solvability distributions in red for the (6, 6, 6) puzzle. We again see a drastic increase in solvability, moving from 0.03 for {\bf Shuffled} up to 0.57 for {\bf L-Tiles}. 
...
To set up a Pretzel challenge, denoted as $(S,V)$, the solver shuffles a deck of $S$ suits, each with $V$ values. They deal the shuffled cards face-up into a grid with $S$ rows, each with $V$ columns. They then remove the lowest valued card of each suit (traditionally aces) and place them in a new column to the left of the existing grid in a prescribed order (traditionally, from top to bottom: spades, hearts, diamonds, clubs). Doing so vacates $S$ cells in the grid, thereafter called holes. A sample Pretzel setup with $S=4$ and $V=4$ is shown in Figure \ref{fig:pretzelbasic}.
...
$\prod_{n = 1}^4n^4(4!)
%(16 \times 12 \times 8 \times 4) \times (12 \times 9 \times 6 \times 3) \times (8 \times 6 \times 4 \times 2) \times (4 \times 3 \times 2 \times 1) 
\approx 10^{11}$.

WHICH 4 IS V and WHICH IS S?
...
\begin{figure}[t]
\includegraphics[width=8.8cm]{pretzel48length.png}
...
\caption{Histograms showing the effect of challenge generation algorithms on minimum solution length.}
...
The algorithm responsible for this result was {\bf Engraved Tiles}. We can see a strong tendency to have shorter solution lengths, with almost 10\% of challenges having a solution length of eight or nine, whereas for {\bf Dominoes}, this is true for only 3\% of challenges. In {\bf Engraved Tiles}, there are five tiles that contain a 0 value; since there will be ten total tiles hidden, on average a challenge will contain 2.5 zeros values. It appears that 0 values are one part of what makes Fujisan challenges interesting.
...
In BoxOff, Rule 1 underscores the importance of adjacent grid squares of the same color, as they are the first moves available in the game. The (4, 6, 4) board contains 38 unique pairs of adjacent grid squares. If we randomly distribute the four colors using the {\bf Shuffled} method, on average a pair will be the same color 22\% of the time. We define this metric as {\it pair equality}. However, if we separate the generated challenges by solvability, we see in Figure \ref{fig:boxoffconnected} that while unsolvable challenges average pair equality is 20\%, for solvable challenges the average increases to 26\%, and this difference is statistically significant.

Through entanglement, the {\bf L-Tiles} constrain adjacent pairs such that at least eight of them are the same color, resulting is a minimum of 21\% pair equality. Thus, L-Tile challenges are more likely to be in the solvable range, as shown in Figure \ref{fig:boxoffconnected}, with 32\% pair equality for unsolvable L-Tile challenges, and 34\% pair equality for those that are solvable.
